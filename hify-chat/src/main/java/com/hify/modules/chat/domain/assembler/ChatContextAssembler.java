package com.hify.modules.chat.domain.assembler;
...
public class ChatContextAssembler {
     ...
     * @return LLM 消息列表
     */
    public List<com.hify.modules.provider.api.dto.chat.ChatMessage> assemble(List<ChatMessage> history, String systemPrompt,
                                          String currentUserMsg, String strategy,
                                          Integer maxContextTokens, Integer maxContextTurns) {
        List<com.hify.modules.provider.api.dto.chat.ChatMessage> result = new ArrayList<>();

        if (systemPrompt != null && !systemPrompt.isBlank()) {
            result.add(new com.hify.modules.provider.api.dto.chat.ChatMessage("system", systemPrompt));
        }
        ...
        for (ChatMessage msg : selected) {
            result.add(new com.hify.modules.provider.api.dto.chat.ChatMessage(msg.getRole(), msg.getContent()));
        }

        result.add(new com.hify.modules.provider.api.dto.chat.ChatMessage("user", currentUserMsg));
        return result;
    }

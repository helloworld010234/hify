...
public class ProviderServiceImpl implements com.hify.modules.provider.api.ProviderService {
    ...
    public Long create(ProviderCreateRequest request) {
        ...
        provider.setApiKey(encryptionService.encrypt(resolveApiKey(request)));
    ...
    public void update(Long id, ProviderUpdateRequest request) {
        ...
        if (provider == null) {
            throw new BizException(ErrorCode.PROVIDER_NOT_FOUND, "供应商不存在: " + id);
        }
        ...
        if (StringUtils.isNotBlank(apiKey)) {
            provider.setApiKey(encryptionService.encrypt(apiKey));
        }
        provider.setAuthConfig(request.getAuthConfig());
        provider.setTimeoutMs(request.getTimeoutMs());
        provider.setMaxRetries(request.getMaxRetries());
    ...
    public ConnectionTestResponse testConnection(Long id) {
        ...
        ConnectionTestResponse result = adapterFactory.getAdapter(provider.getProviderType())
    ...
    /**
     * 解析 API Key：优先取 request.apiKey，为空则从 authConfig 回退。
     */
    private String resolveApiKey(ProviderCreateRequest request) {
        if (StringUtils.isNotBlank(request.getApiKey())) {
            return request.getApiKey();
        }
        if (request.getAuthConfig() != null && StringUtils.isNotBlank(request.getAuthConfig().getApiKey())) {
            return request.getAuthConfig().getApiKey();
        }
        return null;
    }

    private String resolveApiKey(ProviderUpdateRequest request) {
        if (StringUtils.isNotBlank(request.getApiKey())) {
            return request.getApiKey();
        }
        if (request.getAuthConfig() != null && StringUtils.isNotBlank(request.getAuthConfig().getApiKey())) {
